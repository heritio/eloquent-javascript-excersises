...
//Filter function
/*
let arr = [1, 2, 5, 7, 11, 20, 24];
*/
/*
function filter(testArr, testPass) {
  let filteredGreaterThan10 = [];
  for (let element of testArr) {
    if (testPass(element)) {
      filteredGreaterThan10.push(element);
    }
  }
  return filteredGreaterThan10;
}

console.log(filter(arr, (num) => num > 10));

console.log(arr.filter((num) => num % 2 == 0));
*/
/*
function map(prevArr, transform) {
  let transformedArr = [];
  for (let element of prevArr) {
    transformedArr.push(transform(element));
  }
...
console.log(reduce(ourArr, (a, b) => a + b, 0));
*/
...
/*
function filter(ourArr, test) {
  let livingArr = [];
  for (let element of ourArr) {
    if (test(element)) {
      livingArr.push(element);
    }
  }
  return livingArr;
}
...
function map(anotherArr, transform) {
  let transformedArr = [];
  for (let element of anotherArr) {
    transformedArr.push(transform(element));
  }
  return transformedArr;
}
...
*/
function reduce(secondArr, combine, start) {
  let current = start;
...
//This function with deliver an array with an object with true or false after counting elements that are passed through, and deliver a obj like this
//[{name:true, count:1},{name: false, count: 1}] indicating the elements that pass a certain test.
